...
class ReviewRepo {
  ...
  Future<Response> reviewpost(ReviewModel reviewBody) async {
    final response;
    response = await apiClient.postData(
        AppConstants.REVIEWS_PRODUCT_URI, reviewBody.toJson());
    print(response.body);
    return response;
  }
  Future<Response> getReviewList() async {
    return await apiClient.getData(AppConstants.REVIEWS_PRODUCT_URI);
  }
}
